...
const templateData = [
  { name: "Dark Gradient", theme: "violet" },
  { name: "Launch Signal", theme: "ocean" },
  { name: "Editorial Clean", theme: "light" },
  { name: "Ember Release", theme: "ember" },
];
...
export function Dashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-dashboard text-foreground">
      <aside
        className={`sticky top-0 hidden h-screen shrink-0 flex-col border-r border-border/60 bg-sidebar transition-all md:flex ${collapsed ? "w-16" : "w-60"}`}
      >
        ...
        <div className="border-t border-border/60 p-3">
          <div className={`mb-3 min-w-0 ${collapsed ? "hidden" : "block"}`}>
            <p className="truncate text-xs font-medium">{email}</p>
            <p className="mt-1 text-[11px] text-muted-foreground">Free workspace</p>
          </div>
          <Button
            variant="ghost"
            className={`w-full text-muted-foreground ${collapsed ? "px-0" : "justify-start"}`}
            onClick={signOut}
          >
            <LogOut />
            {!collapsed && "Sign out"}
      ...
      <main className="min-w-0 flex-1">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border/60 bg-dashboard/85 px-4 backdrop-blur-xl sm:px-8">
          <div>
            <p className="font-mono text-[10px] uppercase text-primary">Workspace / {view}</p>
          ...
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground sm:flex">
              <span className="size-1.5 rounded-full bg-success" /> Systems operational
            </span>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 lg:grid-cols-3">
        <section className="dash-card lg:col-span-2">
          ...
          <Progress value={(used / limit) * 100} className="mt-7" />
          <div className="mt-3 flex justify-between text-xs text-muted-foreground">
            <span>{limit - used} requests remaining</span>
            <span>Resets Oct 1</span>
          </div>
        </section>
        <section className="dash-card">
          <p className="dash-label">Average render</p>
          ...
          <p className="mt-5 flex items-center gap-2 text-xs text-success">
            <ShieldCheck className="size-4" /> 99.99% API uptime
          </p>
        </section>
      </div>
      <section className="dash-card">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold">Your first image in under a minute</h2>
    ...
    <section className="dash-card">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Use keys from secure server environments only.
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Name this key so you know where it is used.</DialogDescription>
            </DialogHeader>
            {revealed ? (
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  ...
                  <Button size="icon" onClick={() => copy(revealed)} aria-label="Copy new key">
                    {copied === revealed ? <Check /> : <Copy />}
                  </Button>
                </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm">
          <thead className="border-y border-border text-xs text-muted-foreground">
            <tr>
              <th className="py-3 font-medium">Key name</th>
              <th className="font-medium">Key prefix</th>
              ...
              <tr key={key.id} className="border-b border-border/60">
                ...
                <td>
                  <div className="flex justify-end gap-1">
                    ...
                    >
                      {copied.includes(key.last_four) ? <Check /> : <Copy />}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Choose a foundation, then make it yours.</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {templateData.map((item, index) => (
          <button
            ...
            key={item.name}
            className={`template-card ${selected === index ? "border-primary ring-2 ring-primary/20" : "border-border"}`}
          >
            <div className={`template-thumb template-${item.theme}`}>
              <span className="font-mono text-[8px] opacity-70">OGCRAFT / 01</span>
              <strong>
                Build what
                <br />
                comes next.
            ...
            <div className="flex items-center justify-between p-4 text-sm font-medium">
              {item.name}
              {selected === index && <Check className="size-4 text-primary" />}
            </div>
          </button>
        ))}
      </div>
      {selected >= 0 && (
        <section className="dash-card mt-6">
          ...
          <Button className="mt-5">Save template</Button>
        </section>
      )}
    </div>
  );
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <section className="dash-card lg:col-span-2">
        <div className="flex justify-between">
          <div>
            <p className="dash-label">Requests this month</p>
      ...
      <section className="dash-card">
        <p className="dash-label">Top templates</p>
        {["Dark Gradient", "Tech", "Clean White"].map((name, i) => (
          <div key={name} className="mt-5 flex items-center justify-between text-sm">
            <span>{name}</span>
            <span className="text-muted-foreground">{[19, 14, 9][i]} renders</span>
          </div>
        ))}
      </section>
    </div>
...
function Billing() {
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="dash-card">
        <p className="dash-label">Current plan</p>
        <div className="mt-5 flex items-end justify-between">
          <div>
            <p className="text-3xl font-semibold">Free</p>
            <p className="mt-1 text-sm text-muted-foreground">100 images each month</p>
          </div>
          <span className="rounded-full bg-primary/15 px-3 py-1 text-xs text-primary">Active</span>
        </div>
        <Button className="mt-8">Upgrade to Pro</Button>
      </section>
      <section className="dash-card">
        <p className="dash-label">Included</p>
